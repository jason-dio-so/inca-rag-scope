...
class Step9Loader:
    ...
    def truncate_fact_tables(self):
        """Truncate 4 fact tables (clear+reload)"""
        logger.info("Truncating fact tables (clear+reload mode)...")
        ...
        self.conn.commit()
        logger.info("Fact tables truncated")
    ...
    def load_coverage_canonical(self, excel_path: Path):
        ...
        self.conn.commit()
        logger.info(f"Inserted {inserted} rows into coverage_canonical")
    ...
    def load_coverage_instance(self, insurer_key: str, scope_mapped_path: Path):
        ...
        # Schema: instance_id (auto), insurer_id, product_id, variant_id, coverage_code,
        ...
        for row in rows:
            ...
            # coverage_code must exist in coverage_canonical if mapping_status=matched
            if mapping_status == 'matched' and not coverage_code:
                logger.warning(f"Matched coverage without code: {coverage_name_raw}, skipping")
                skipped += 1
                continue

            # coverage_code is NOT NULL in schema - use placeholder for unmatched
            if not coverage_code:
                # Generate placeholder coverage_code for unmatched (e.g., Z9999)
                # Skip unmatched rows since we can't assign a valid FK
                logger.debug(f"Skipping unmatched coverage (no code): {coverage_name_raw}")
                skipped += 1
                continue
            ...
            self.cursor.execute(
                ...
                (
                    ...
                    product_id,
                    variant_id,
                    coverage_code,  # Already validated as not None above
                    coverage_name_raw,
                    source_page_int,
                    mapping_status,
        ...
        self.conn.commit()
        logger.info(f"  Inserted {inserted} rows (skipped {skipped}) into coverage_instance for {insurer_key}")
    ...
    def load_amount_fact(self, insurer_key: str, cards_path: Path):
        ...
        for line in lines:
            ...
            insert_sql = """
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (coverage_instance_id) DO NOTHING
            """
        ...
        self.conn.commit()
        logger.info(f"  Inserted {inserted} amount facts (skipped {skipped}) for {insurer_key}")
    ...
    def run(self, mode: str = 'clear_reload'):
        """
        Main execution flow
        """
        ...
        try:
            ...
            # Step 2: Truncate fact tables (clear+reload)
            if mode == 'clear_reload':
...
def main():
    ...
    parser.add_argument(
        '--mode',
        default='clear_reload',
        choices=['clear_reload'],
        help='Load mode (clear_reload only)'
    )
    parser.add_argument(
        '--project-root',
        type=Path,
