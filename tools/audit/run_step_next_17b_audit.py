...
def generate_amount_status_dashboard(insurers: List[str]) -> str:
    """
    STEP NEXT-18X: Generate AMOUNT_STATUS_DASHBOARD.md (IN-SCOPE KPI only)

    KPI Scope:
    - IN-SCOPE: mapping_status == "matched" (canonical coverage_code mapped)
    - OUT-OF-SCOPE: mapping_status != "matched" (unmapped, structural outliers)

    Structural outliers (heungkuk/hanwha) reported separately, NOT contaminating KPI.
    """
    report_lines = [
    ...
    in_scope_totals = Counter()
    out_scope_totals = Counter()
    insurer_details = {}

    for insurer in insurers:
        cards = parse_coverage_cards(insurer)
        in_scope_stats = Counter()
        out_scope_stats = Counter()
        out_scope_details = []

        for card in cards:
            status = card.get("amount", {}).get("status", "NOT_AVAILABLE")
            mapping_status = card.get("mapping_status", "").strip().lower()
            coverage_name = card.get("coverage_name_raw", "")

            if mapping_status == "matched":
                # IN-SCOPE
                in_scope_stats[status] += 1
                in_scope_totals[status] += 1
            else:
                # OUT-OF-SCOPE
                out_scope_stats[status] += 1
                out_scope_totals[status] += 1
        ...
        # IN-SCOPE row
        in_scope_total = sum(in_scope_stats.values())
        in_scope_confirmed = in_scope_stats.get("CONFIRMED", 0)
        in_scope_unconfirmed = in_scope_stats.get("UNCONFIRMED", 0)
        in_scope_confirmed_pct = (in_scope_confirmed / in_scope_total * 100) if in_scope_total > 0 else 0.0

        insurer_details[insurer] = {
            "in_scope": in_scope_stats,
            "out_scope": out_scope_stats,
            "out_scope_details": out_scope_details
        }

        report_lines.append(
            f"| {insurer} | {in_scope_confirmed} | {in_scope_unconfirmed} | {in_scope_total} | {in_scope_confirmed_pct:.1f}% |"
        )

    # IN-SCOPE TOTAL row
    in_scope_total_all = sum(in_scope_totals.values())
    in_scope_total_confirmed = in_scope_totals.get("CONFIRMED", 0)
    in_scope_total_unconfirmed = in_scope_totals.get("UNCONFIRMED", 0)
    in_scope_total_confirmed_pct = (in_scope_total_confirmed / in_scope_total_all * 100) if in_scope_total_all > 0 else 0.0

    report_lines.append(
        f"| **TOTAL** | **{in_scope_total_confirmed}** | **{in_scope_total_unconfirmed}** | **{in_scope_total_all}** | **{in_scope_total_confirmed_pct:.1f}%** |"
    )
    ...
    for insurer in insurers:
        ...
        out_scope_stats = details["out_scope"]
        out_scope_total = sum(out_scope_stats.values())
        out_scope_confirmed = out_scope_stats.get("CONFIRMED", 0)
        out_scope_unconfirmed = out_scope_stats.get("UNCONFIRMED", 0)

        report_lines.append(
            f"| {insurer} | {out_scope_confirmed} | {out_scope_unconfirmed} | {out_scope_total} |"
        )

    out_scope_total_all = sum(out_scope_totals.values())
    out_scope_total_confirmed = out_scope_totals.get("CONFIRMED", 0)
    out_scope_total_unconfirmed = out_scope_totals.get("UNCONFIRMED", 0)
    ...
    for insurer in structural_outliers:
        if insurer in insurer_details:
            ...
            if details:
                report_lines.append(f"### {insurer.upper()} OUT-OF-SCOPE Detail (first 5)")
                report_lines.append("")
