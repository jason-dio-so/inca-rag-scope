# STEP NEXT - 완료 보고서 아카이브

**Archive Date**: 2025-12-29
**Coverage**: STEP NEXT-4 ~ STEP NEXT-9.1 (2025-12-27 ~ 2025-12-28)

> 이 파일은 STEP NEXT-10B 이전의 완료 보고서를 보관합니다.
> 최신 상태는 `STATUS.md`를 참조하세요.

---
**현재 Loader 동작**:
1. `coverage_cards.jsonl` 읽기
2. `card['amount']` 필드 확인
3. 필드 있음 → 직접 매핑
4. 필드 없음 → `UNCONFIRMED` + NULL 저장

**DB 상태 예상**:
- 현재 `amount_fact`의 기존 행은 "추출된 값" (오염됨)
- 재적재 필요: 전체 `UNCONFIRMED` + NULL (Step7 미실행이므로 정상)

**다음 STEP (NEXT-10B-3)**:
- DB 재적재 (reset_then_load)
- SQL Audit 실행
- amount_fact 전체 UNCONFIRMED 확인

**참고 문서**:
- Loader 소스: `apps/loader/step9_loader.py`
- Guardrail 테스트: `tests/test_lineage_lock_loader.py`
- Audit: `docs/audit/STEP_NEXT_10B_2_LOADER_AUDIT.md`
- Freeze tag: `freeze/pre-10b2-20251228-2303`
- Work branch: `fix/10b2-loader-lineage-safe`

---

## STEP NEXT-10B-1A 완료 (2025-12-28)

**목표**: Step7 소스 복구 + 계보 고정 + 혼입 방지 Guardrail 설치

**문제 상황**:
- `pipeline/step7_amount`, `pipeline/step7_amount_integration`에 .py 소스 없이 .pyc만 존재
- Git 히스토리에도 없음 → 소스가 한 번도 커밋되지 않음 (Branch C)
- 계보 불명확 → DB 오염 위험

**완료 내용**:
1. **진단 (STEP 0)**:
   - 소스 탐색/Git 히스토리 분석 → Branch C 확정 (소스 미커밋)
   - 진단 보고서: `docs/audit/STEP_NEXT_10B_1A_STEP7_DIAG.md`

2. **소스 복구 (STEP 1)**:
   - Branch C 프로토콜에 따라 minimal stub 구현
   - 생성된 파일:
     - `pipeline/step7_amount/__init__.py`
     - `pipeline/step7_amount/run.py` (공식 엔트리포인트)
...
3. **엔트리포인트 고정 (STEP 2)**:
   - `pipeline/step7_amount/run.py`를 단일 실행 경로로 고정
   - `--dry-run` 지원 (파일/DB 변경 없이 계획만 출력)
   - 실행 검증:
     ```bash
     python -m pipeline.step7_amount.run --insurer samsung --dry-run
     ```

4. **Guardrail 설치 (STEP 3)**:
   - 테스트: `tests/test_lineage_lock_step7.py` (5개 테스트)
     - 소스 존재 검증
     - 금지 import 차단 (inca-rag-final, apps.loader, apps.api)
     - 엔트리포인트 실행 검증
     - dry-run 안전성 검증
   - 문서: `docs/guardrails/LINEAGE_LOCK.md`
     - 공식 실행 경로 정의
     - 금지 import 목록
     - 변경 통제 절차
     - 사고 대응 절차
...
5. **검증 (STEP 4)**:
   - ✅ pytest 전체 PASS (101 passed)
   - ✅ lineage lock 테스트 PASS (5 passed)
   - ✅ `run.py --help` 정상 동작
   - ✅ `run.py --dry-run` 정상 동작
   - ✅ 소스 파일 존재 확인 (4개 .py in step7_amount, 2개 .py in step7_amount_integration)
   - ✅ .pyc 캐시 정리 완료

**금지 사항 강제**:
- inca-rag-final / inca_rag_final import 금지
- apps.loader.* / apps.api.* import 금지
- 위반 시 테스트 FAIL + ImportError 발생

**다음 STEP (NEXT-10B-2)**:
- Step7 로직 정교화 (amount 추출 규칙 구현)
- Loader 수정 (amount_fact 적재)
- DB 재적재 + Audit

**참고 문서**:
- 진단: `docs/audit/STEP_NEXT_10B_1A_STEP7_DIAG.md`
- Guardrail: `docs/guardrails/LINEAGE_LOCK.md`
